...
Upload_Timestamp=config["Upload_Timestamp"]
...
if os.path.exists(duplicate_output_path):
    ...
    Combined_Duplicate_Output = pd.concat([
        Duplicate_Output_v2[['Primary_DocName_Page', 'Exception_Category',"Exception_ID"]].rename(columns={'Primary_DocName_Page': 'DocName'}),
        Duplicate_Output_v2[['Secondary_DocName_Page', 'Exception_Category',"Exception_ID"]].rename(columns={'Secondary_DocName_Page': 'DocName'})
    ])
    Combined_Duplicate_Output.columns=["File_Name","Exception_Category","Exception_ID"]
    Combined_Duplicate_Output.drop_duplicates(subset=["File_Name"],inplace=True)
    Combined_Duplicate_Output["Duplicate_Exception_Flag"]="Yes"

else:
    print("Duplicate check not selected or No Duplicate Forgery found.")
...
if os.path.exists(metadata_output_path):
    ...
    print("MetaData check not selected or No Meta Data forgery found.")
...
if os.path.exists(Pdf_Edit_output_path):
    ...
    # PDF_Edit_Output=pd.read_excel(Pdf_Edit_output_path)
    print("Pickle file loaded successfully.")
    ...
    print("PDF Edit check not selected or No PDF Edit forgery found.")
    
# PDF_Edit_Output.to_excel("C:/MSIL/Protiviti-Claim-Service-enterprise-app/server/uploads/PROTIVITY_AJain_1756098493_1756098744326/pdf_output.xlsx")
...
# base_data_v1.to_excel("C:/MSIL/Protiviti-Claim-Service-enterprise-app/server/uploads/PROTIVITY_AJain_1756098493_1756098744326/base_data.xlsx")
...
if os.path.exists(Image_Edit_output_path):
    ...
    Image_Edit_Output=Image_Edit_Output.loc[Image_Edit_Output["Image_Edit_Exception_Flag"]=="Yes",:]
    ...
    print("Image Edit check not selected or No Image Edit forgery found.")
...
if os.path.exists(Copy_Move_output_path):
    ...
    Copy_Move_Output=Copy_Move_Output.loc[Copy_Move_Output["Copy_Move_Exception_Flag"]=="Yes",:]
    ...
    print("Copy Move check not selected or No Copy Move forgery found.")
...
def QR_Rules(rows):
    if rows["URL_Present"]=="Yes" and rows["URL_Open"]=="No":
        return "Yes"
    elif rows["Text Length"] <5:
        return "Yes"
    elif rows["Text Length"]>=100 and rows["URL_Present"]=="No":
        return "Yes"
    else:
        return "No"
...
if os.path.exists(QR_Code_output_path):
    ...
    QR_Code_Output["URL_Present"]=QR_Code_Output['Has Web Link'].apply(lambda x:"Yes" if x=="Yes" else "No")
    QR_Code_Output["URL_Open"]=QR_Code_Output['HTTP Status'].apply(lambda x:"Yes" if x==200 else "No")
    QR_Code_Output["QR_Code_Exception_Flag"]=QR_Code_Output[["URL_Present","URL_Open","Text Length"]].apply(QR_Rules,axis=1)
    QR_Code_Output=QR_Code_Output.loc[QR_Code_Output["QR_Code_Exception_Flag"]=="Yes",:]
    QR_Code_Output["File_Name"] = QR_Code_Output["File Name"].apply(lambda x: re.sub(r"\^page\d+\.jpg$", ".pdf", x))    
    QR_Code_Output.drop_duplicates(subset=["File_Name","QR_Code_Exception_Flag"],inplace=True)
    print("Pickle file loaded successfully.")
else:
    print("QR Edit forgery check not selected or No QR Edit forgery found.")
...
base_data_v1["Upload_Timestamp"]=Upload_Timestamp

# base_data_v1["Session_ID"]="msil_il_143278"
# base_data_v1["Employee_ID"]="M43267"
# base_data_v1["Upload_Timestamp"]="7/1/2025  9:35:34 AM"
...
if 'Duplicate_Output_v2' in locals() and  isinstance(Duplicate_Output_v2, pd.DataFrame):
    ...
    Duplicate_Output_v2["Upload_Timestamp"]=Upload_Timestamp
    ...
    cols=['Employee_ID','Session_ID','Exception_ID','Exception_Category', 'Primary_Document', 'Secondary_Document', 'Percentage_Match','Upload_Timestamp']
    Duplicate_Output_v2=Duplicate_Output_v2.loc[:,cols]
    Duplicate_insert_query = """
    INSERT INTO Duplicate_Table
    (Employee_ID, Session_ID,Exception_ID, Exception_Category, Primary_Document, Secondary_Document, Percentage_Match,Upload_Timestamp)
    VALUES (%s, %s, %s, %s, %s, %s, %s,%s)
    """  
    ...
    print("No Forgery found or Duplicate check not selected.")

# /***********************************MetaData Table Inserstion*****************/
if 'MetaData_Output' in locals() and  isinstance(MetaData_Output, pd.DataFrame):
    cols=['Employee_ID',
          'Session_ID',
          'File_Name',
    ...
    MetaData_Output_v1["Upload_Timestamp"]=Upload_Timestamp
    MetaData_Output_v1["Document_Type"]="Digital"
    ...
    Producer, Software, Modified_Date, Title, Author, Document_Type,Upload_Timestamp)
    ...
    print("No Forgery found or MetaData check not selected.")

# /***********************************PDF Edit Table Inserstion*****************/
if 'PDF_Edit_Output' in locals() and  isinstance(PDF_Edit_Output, pd.DataFrame):
    PDF_Edit_Output["Employee_ID"]=Employee_ID
    PDF_Edit_Output["Session_ID"]=Session_ID
    PDF_Edit_Output["Upload_Timestamp"]=Upload_Timestamp
    ...
    cols=["Employee_ID","Session_ID",'Input_File_Name','Output_File_Name','PDF_Edited_Flag','Edited_Text','Upload_Timestamp']
    ...
    PDF_Edited_Flag, Edited_Text,Upload_Timestamp)
    ...
    print("No Forgery found or PDF Edit check not selected.")
    
    
# /***********************************Image Edit Table Inserstion*****************/
if 'Image_Edit_Output' in locals() and  isinstance(Image_Edit_Output, pd.DataFrame):
    Image_Edit_Output["Employee_ID"]=Employee_ID
    Image_Edit_Output["Session_ID"]=Session_ID
    Image_Edit_Output["Upload_Timestamp"]=Upload_Timestamp
    Image_Edit_Output_v1=Image_Edit_Output.rename(columns={"Image_Edit_Exception_Flag":"Image_Edit_Flag"})
    Image_Edit_Output_v1["Input_File_Name"]=Image_Edit_Output_v1["File_Name"].apply(lambda x:x)
    Image_Edit_Output_v1["Output_File_Name"]=Image_Edit_Output_v1["File_Name"].apply(lambda x:x)
    cols=["Employee_ID","Session_ID",'Input_File_Name','Output_File_Name','Image_Edit_Flag','Upload_Timestamp']
    Image_Edit_Output_v1=Image_Edit_Output_v1.loc[:,cols]
    Image_Edit_insert_query = """
    INSERT INTO Image_Edit_Table
    (Employee_ID, Session_ID, Input_File_Name, Output_File_Name,
    Image_Edit_Flag,Upload_Timestamp)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.executemany(Image_Edit_insert_query, Image_Edit_Output_v1.to_records(index=False).tolist())
    # Image_Edit_Output_v1.to_excel("C:/MSIL/Enterprise Application/Session_ID/Excel Output/test4.xlsx")
else:
    print("No Forgery found or Image Edit check not selected.")    


# /***********************************Copy Move Table Inserstion*****************/
if 'Copy_Move_Output' in locals() and  isinstance(Copy_Move_Output, pd.DataFrame):
    Copy_Move_Output["Employee_ID"]=Employee_ID
    Copy_Move_Output["Session_ID"]=Session_ID
    Copy_Move_Output["Upload_Timestamp"]=Upload_Timestamp
    Copy_Move_Output_v1=Copy_Move_Output.rename(columns={"Copy_Move_Exception_Flag":"Copy_Move_Edit_Flag"})
    Copy_Move_Output_v1["Input_File_Name"]=Copy_Move_Output_v1["File_Name"].apply(lambda x:x)
    Copy_Move_Output_v1["Output_File_Name"]=Copy_Move_Output_v1["File_Name"].apply(lambda x:x.rsplit('.',1)[0]+".jpg")
    cols=["Employee_ID","Session_ID",'Input_File_Name','Output_File_Name','Copy_Move_Edit_Flag','Upload_Timestamp']
    Copy_Move_Output_v1=Copy_Move_Output_v1.loc[:,cols]
    Copy_Move_insert_query = """
    INSERT INTO Copy_Move_Edit_Table
    (Employee_ID, Session_ID, Input_File_Name, Output_File_Name,
    Copy_Move_Edit_Flag,Upload_Timestamp)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.executemany(Copy_Move_insert_query, Copy_Move_Output_v1.to_records(index=False).tolist())
    # Copy_Move_Output_v1.to_excel("C:/MSIL/Enterprise Application/Session_ID/Excel Output/test5.xlsx")

else:
    print("No Forgery found or Copy Move check not selected.")    
    
    
# /***********************************QR Code Table Inserstion*****************/
if 'QR_Code_Output' in locals() and  isinstance(QR_Code_Output, pd.DataFrame):
    QR_Code_Output["Employee_ID"]=Employee_ID
    QR_Code_Output["Session_ID"]=Session_ID
    QR_Code_Output["Upload_Timestamp"]=Upload_Timestamp
    QR_Code_Output_v1=QR_Code_Output.rename(columns={"Text":"QR_Data","QR_Code_Exception_Flag":"QR_Edit_Flag"})
    QR_Code_Output_v1["Input_File_Name"]=QR_Code_Output_v1["File_Name"].apply(lambda x:x)
    cols=["Employee_ID","Session_ID",'Input_File_Name','QR_Data','URL_Present','URL_Open','QR_Edit_Flag','Upload_Timestamp']
    QR_Code_Output_v1=QR_Code_Output_v1.loc[:,cols]
    QR_Code_insert_query = """
    INSERT INTO QR_Code_Edit_Table
    (Employee_ID, Session_ID, Input_File_Name, QR_Data, URL_Present,
    URL_Open,QR_Edit_Flag,Upload_Timestamp)
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    cursor.executemany(QR_Code_insert_query, QR_Code_Output_v1.to_records(index=False).tolist())
    # QR_Code_Output_v1.to_excel("C:/MSIL/Enterprise Application/Session_ID/Excel Output/test6.xlsx")

else:
    print("No Forgery found or QR Code check not selected.")        

# /***********************************Transaction Table Inserstion*****************/
Transaction_insert_query = """
INSERT INTO Transaction_Output
(Employee_ID,Session_ID,Upload_Timestamp,Document_Name,
Duplicate_Exception_Flag,PDF_Edit_Exception_Flag,
MetaData_Exception_Flag,Copy_Move_Exception_Flag ,
Image_Edit_Exception_Flag,QR_Code_Exception_Flag ,Risk_Flag,Exception_ID)
VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
"""
