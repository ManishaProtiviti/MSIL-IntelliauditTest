...
const name = 'QR Edit Forgery'
const folders = ["Input Files", "Image Files", "Excel Files"]

export async function qrCodeCheck(requestId, checksJsonColl) {
  ...
  const scriptPath = path.join(process.cwd(), "scripts","dist", checksMappingWithPythonFile.qrCode);
  ...
  return result;  
    ...
    console.log("Error in QR Edit Forgery script", err);
  }
}