...
const name = 'Copy Move Forgery'
const folders = ["Input Files", "Image Files", "Excel Files"]

export async function copyMoveForge(requestId, checksJsonColl) {
  ...
  const scriptPath = path.join(process.cwd(), "scripts","dist", checksMappingWithPythonFile.copyMoveForge);
  const data = JSON.stringify(checksJsonColl[name]);
  const result = await runPythonScript(pythonExec, scriptPath, data);
  console.log('copyMoveForge python script output:', result)
  return result;   
  }
  catch(err){
    console.log("Error in Copy Move Forgery script", err);
  }
}